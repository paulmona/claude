...
Required for: bootstrap, next, status, bug, enhancement, bughunt.
...
Enable the [Notion MCP integration](https://www.notion.so/integrations) in your Claude Code settings. This gives Claude read/write access to your Notion workspace via MCP tools (`notion-search`, `notion-create-pages`, `notion-update-page`, `notion-fetch`, `notion-create-database`).
...
- PRD Database ID: your-database-id-here
```

To find the database ID: open the database in Notion as a full page, copy the URL. The ID is the 32-character hex string before the `?v=` parameter:
```
