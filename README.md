...
### pm-help Auto-Sync (GitHub Actions)

A GitHub Actions workflow (`.github/workflows/sync-pm-help.yml`) automatically keeps `/pm-help` up to date as a safety net.

**Trigger:** Any push that modifies `skills/pm-*.md` files (excluding `pm-help.md` itself to avoid loops).

**What it does:**
1. Compares all `skills/pm-*.md` files against the table in `skills/pm-help.md`
2. If any skill is missing from the table, invokes Claude (via `claude-code-action`) to read the new skill and add it
3. Commits the update automatically

**Two-layer approach:**
- **Layer 1 (convention):** The CLAUDE.md generated by `/pm-bootstrap` includes a "Skill Maintenance" rule telling Claude to update `pm-help.md` whenever creating a new skill. This handles 95% of cases at zero cost.
- **Layer 2 (CI guardrail):** This workflow catches anything Layer 1 missed. Requires an `ANTHROPIC_API_KEY` secret in the repo (API billing, not Max plan).

**Setup:** Add your Anthropic API key as a repository secret named `ANTHROPIC_API_KEY`. See the [Claude Code GitHub Actions docs](https://code.claude.com/docs/en/github-actions) for details.

**Cost note:** This uses the Anthropic API (per-token billing), not your Max subscription. The workflow uses Sonnet with `--max-turns 5`, so each run costs a few cents at most. It only triggers when skill files actually change.
