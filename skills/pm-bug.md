...
If `.claude/bug-state.json` exists on start, present the saved info:
```
Found bug report draft in progress:
  Summary: [bug summary]
  Repo: [repo]
  Milestone: [milestone or "not yet selected"]
  Severity: [severity or "not yet selected"]

  Resume and create this bug, or start fresh?
```

State is saved after info gathering (Step 2) so the user can pause before creation.
State file is deleted after the bug is created.
...
2. Ask for the following if not clear from the description:
   ...
   **Save state** after all info is gathered. Offer: "Ready to create the bug in GitHub, or save & exit to finish later?"
...
6. **Delete state file.** Confirm: "Bug #[N] created: [title]. Run /pm-next [repo] to see if this affects current priorities."
